...
class FunSetSuite extends FunSuite {
  ...
  test("forall Returns whether all bounded integers within `s` satisfy `p`.") {
    new TestSets {
      ...
      assert(forall(t, ((x: Int)=> x > 0)) === true, "forall 1 2 3 > 0")
  ...
  test("exists Returns whether there exists a bounded integer within `s` that satisfies `p`.") {
    new TestSets {
      ...
      assert(exists(t, s1) === true, "exists s1")
      assert(exists(t, s2) === true, "exists s2")
      assert(exists(t, s3) === true, "exists s3")
      assert(exists(t, (x: Int) => x == 0) === false, "not exists 0")
    }
  }
}
